---
name: orchestration
description: "MANDATORY before every engineering or repository task, including read-only reconnaissance, investigation, review, testing, and implementation: route work across Sol, Terra, Luna, and Spark; apply the Mandatory Spark gate before every implementation."
---
...
- [Routing details](references/routing-details.md): roles, handoffs, escalation,
  parallelism, and workflow selection.
- [CLI workers](references/cli-workers.md): required whenever Luna or Spark is
  not exposed by the native subagent tool.
- [Acceptance workflows](references/acceptance-workflows.md): required for bug,
...
- Treat existing tests as contracts. Do not weaken their expectations merely to
  pass; change them only when the task/specification justifies it or their
  prior expectation is demonstrated invalid.
...
[CLI workers](references/cli-workers.md), verify the exact model in the live
